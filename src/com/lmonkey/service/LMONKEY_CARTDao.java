...
public class LMONKEY_CARTDao {
	public static int insert(LMONKEY_CART cart){
		String sql = "insert into hogenshop_cart values(null,?,?,?,?,?,?,?,1)";
	...
	public static LMONKEY_CART getCartShop(String id) {
		...
		try {
			String sql = "select * from hogenshop_CART where CART_ID=? and CART_VALID=1 order by CART_ID desc";
	...
	public static LMONKEY_CART getCartShop(String uid, String pid) {
		...
		try {
			String sql = "select * from hogenshop_CART where CART_U_ID=? and CART_P_ID=? and CART_VALID=1 order by CART_ID desc";
			ps = conn.prepareStatement(sql);
			ps.setString(1, uid);
	...
	public static ArrayList<LMONKEY_CART> getCart(String id){
		...
		try {
			String sql = "select * from hogenshop_CART where CART_U_ID=? and CART_VALID=1 order by CART_ID desc";
			ps = conn.prepareStatement(sql);
			ps.setString(1, id);
			rs = ps.executeQuery();
	...
	public static int updatenum(int esid, int count){
		...
		String sql = "update hogenshop_cart set cart_quantity=? where cart_id=? ";
	...
	public static int getDeleteDD(int id) {
		String sql="delete from hogenshop_cart where cart_id=?";
