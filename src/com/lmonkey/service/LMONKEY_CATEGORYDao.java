...
public class LMONKEY_CATEGORYDao {
	public static ArrayList<LMONKEY_CATEGORY> selectAll() {
		...
		try {
			String sql = "select * from hogenshop_CATEGORY";
	...
	public static ArrayList<LMONKEY_CATEGORY> selectCat(String flag) {
		...
		try {
			...
			if(flag!=null && flag.equals("father")){
				...
				sql = "select * from hogenshop_CATEGORY where CATE_PARENT_ID=0";
			}else{
				sql = "select * from hogenshop_CATEGORY where CATE_PARENT_ID!=0";
			}
			ps = conn.prepareStatement(sql);
	...
	public static LMONKEY_CATEGORY selectById(int id) {
		...
		try {
			String sql = "select *  from hogenshop_CATEGORY  where CATE_ID=?";
	...
	public static int insert(LMONKEY_CATEGORY cate) {
		String sql = "insert into hogenshop_category values(null, ?, ?)";
	...
	public static int update(LMONKEY_CATEGORY cate) {
		String sql = "update hogenshop_CATEGORY set CATE_NAME=?, CATE_PARENT_ID=?  where CATE_ID = ?";
	...
	public static int del(int id) {
		String sql = "delete from hogenshop_CATEGORY where CATE_ID=?";
